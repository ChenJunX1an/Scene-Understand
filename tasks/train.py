...
def train(
    ...
    for i, (media_type, batch) in enumerate(iterator):
        ...
        #         # 打印键和对应张量的形状
        #         print(f"Train Key: {key}, Shape: {value.shape}")
        #     else:
        #         # 如果不是张量，打印键和对应值的类型及长度
        #         print(f"Train Key: {key}, Type: {type(value)}, Length: {len(value)}, value:{value}")
        #         break
        # else:
        #     print("Train Batch is not a dictionary.")
        ####################test##########
...
def evaluate_all(
    ...
    model.eval()
    model_without_ddp.custom_model.config.use_cache = True
    val_scores = {}
    ...
    model.train()
    model.module.custom_model.config.use_cache = False
    return val_scores
